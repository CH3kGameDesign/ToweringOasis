...
public class Setting : BaseState
{
    ...
    private void GetResolutions()
    {
        ...
        for (int i = 0; i < m_ResolutionDropdownMenu.options.Count; i++)
        {
            if (Screen.width == m_Resolutions[i].width && Screen.height == m_Resolutions[i].height)
            {
                ...
                //m_ResolutionDropdownMenu.GetComponent<EditorLabel> = m_Resolutions[i].width.ToString() + "x" + m_Resolutions[i].height.ToString();
            }
        }
    }
	...
	{
		GameManager.Instance.m_PrevMenu.SetActive(true);
        m_ThisMenu.SetActive(false);
		GameManager.Instance.m_PrevMenu = null;
	}
}